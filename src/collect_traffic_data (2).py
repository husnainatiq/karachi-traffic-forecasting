"""
Karachi Traffic Congestion Forecasting - Data Collection Script (Week 2)

What this script does, step by step:
1. Reads the list of 10 routes from data/raw/routes.csv
2. For each route, calls the Google Maps Directions API ONCE
   (with departure_time=now), which returns both:
   - duration: travel time with no traffic (typical/free-flow)
   - duration_in_traffic: travel time right now, with live traffic
3. Calculates a congestion_index = duration_in_traffic / duration
...
# Step 2: Load your Google Maps API key from the .env file
load_dotenv()
GOOGLE_MAPS_API_KEY = os.getenv("GOOGLE_MAPS_API_KEY")

if not GOOGLE_MAPS_API_KEY:
    raise ValueError(
        "GOOGLE_MAPS_API_KEY not found. Make sure your .env file has a line like:\n"
...
def get_travel_times(origin_lat, origin_lon, dest_lat, dest_lon):
    """
    Calls the Google Maps Directions API ONCE for one origin -> destination pair.
    Using departure_time=now tells Google to include live traffic data.
    Returns a tuple: (duration_seconds, duration_in_traffic_seconds)
    - duration_seconds: typical/free-flow travel time
    - duration_in_traffic_seconds: travel time right now, with live traffic
    Returns (None, None) if the call failed.
    """
    url = "https://maps.googleapis.com/maps/api/directions/json"
    params = {
        "origin": f"{origin_lat},{origin_lon}",
        "destination": f"{dest_lat},{dest_lon}",
        "departure_time": "now",       # this is what activates live traffic data
    ...
    try:
        ...
        if data.get("status") != "OK":
            # Google returns a status field even on "successful" HTTP responses -
        ...
        # duration_in_traffic may be missing in rare cases, so use .get() safely
        duration_in_traffic_seconds = leg.get("duration_in_traffic", {}).get("value")

        return duration_seconds, duration_in_traffic_seconds

    except Exception as e:
        logger.error(f"Failed to fetch travel time for "
                     f"{origin_lat},{origin_lon} -> {dest_lat},{dest_lon}: {e}")
        return None, None
...
def collect_data_for_all_routes():
    ...
    for route in routes:
        try:
            ...
            free_flow_time, traffic_time = get_travel_times(
...
def save_results(results):
    ...
    with open(OUTPUT_FILE, mode="a", newline="", encoding="utf-8") as f:
        fieldnames = [
            "timestamp", "route_id", "route_name", "corridor",
